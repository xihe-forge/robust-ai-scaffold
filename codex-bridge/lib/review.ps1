# Codex Bridge - Review Helpers
# Provides functions for Claude (Opus) to review Codex's changes

function Get-CodexReviewSummary {
    <#
    .SYNOPSIS
    Generates a review summary of Codex's changes for Claude to evaluate.
    .PARAMETER Result
    The result hashtable from Invoke-Codex.
    ...
    if ($Result.Status -eq "error" -or $Result.Status -eq "timeout") {
        $review.Blockers = "Codex execution failed with status: $($Result.Status). Check log: $($Result.LogFile)"
        return $review
    }

    if ($Result.Status -eq "no-changes") {
        $review.Blockers = "Codex made no changes. Task may need re-specification."
        return $review
    }

    if ($Result.Status -eq "commit-failed") {
        $review.Blockers = "Codex made changes but auto-commit failed. Check log: $($Result.LogFile)"
        return $review
    }

    # Get diff stats
    try {
        $review.DiffStats = Get-WorktreeDiff -BranchName $Result.Branch -ProjectRoot $ProjectRoot -StatOnly
    } catch {
    ...
    # Get full diff
    try {
        $review.FullDiff = Get-WorktreeDiff -BranchName $Result.Branch -ProjectRoot $ProjectRoot
    } catch {
        $review.Blockers = "Could not read full diff: $_"
        return $review
    }

    # Check for blocker file from Codex
    $blockerFile = Join-Path $Result.WorktreePath ".codex-blockers.md"
...
function Format-ReviewForClaude {
    <#
    .SYNOPSIS
    Formats review data into a readable report for Claude to evaluate.
    .PARAMETER Review
    Review hashtable from Get-CodexReviewSummary.
    .OUTPUTS
    ...
    [void]$sb.AppendLine("## Codex Execution Review: $($Review.TaskId)")
    [void]$sb.AppendLine("")
...
function Complete-CodexTask {
    <#
    .SYNOPSIS
    Finalizes a Codex task: merges changes and cleans up worktree.
    .PARAMETER Result
    The result hashtable from Invoke-Codex.
    .PARAMETER Verdict
    ...
    if ($Verdict -eq "accept") {
        # Guard: only allow merge for success status (#10, #R4-4)
        if (-not $Result.Status -or $Result.Status -ne "success") {
            ...
            $outcome.Error = "Refusing to merge: Codex result status is '$($Result.Status)', not 'success'"
            return $outcome
        }

        # Merge Codex's branch
        $mergeResult = Merge-CodexWorktree -BranchName $Result.Branch -TaskId $Result.TaskId -ProjectRoot $ProjectRoot
    ...
    # Cleanup worktree
    Remove-CodexWorktree -TaskId $Result.TaskId -ProjectRoot $ProjectRoot
